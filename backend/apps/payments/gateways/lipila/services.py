...
class LipilaGateway:
    ...
    apps/payments/gateways/base.py. See that module's docstring — this is
    the "real" counterpart to gateways/console.py.
    """
    ...
    def create_mobile_collection(self, *, payment, phone_number, callback_url):
        payload = {
            ...
            # payment.reference (also sent as referenceId) is stable from
            # the moment payment starts, so it's what identifies this
            # payment everywhere, including the webhook's fallback lookup
            # — unlike a registration/team's reference number, which
            # doesn't exist yet at this point (only assigned once
            # confirmed; see BaseRegistration.save()).
            "narration": f"{settings.EVENT_NAME} — {payment.reference}",
    ...
    def create_card_collection(self, *, payment, participant, city, address, zip_code, country, back_url, callback_url):
        payload = {
            ...
            "collectionRequest": {
                ...
                # payment.reference, not payment.target.registration_number
                # — the same "doesn't exist yet at initiate time" reason as
                # create_mobile_collection above.
                "narration": f"{settings.EVENT_NAME} {payment.reference}",
                "backUrl": back_url or "",
                "referenceData": payment.reference,
            },
